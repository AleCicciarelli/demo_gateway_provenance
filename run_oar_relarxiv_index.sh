#!/usr/bin/env bash
# Submit from the repository root: oarsub -l /gpu=1,walltime=48:0:0 'bash ./run_oar_relarxiv_index.sh'
set -euo pipefail
...
OUTPUT="${ROW_INDEX_FOLDER:-$PWD/faiss_index_relarxiv_rows_bge_m3_semantic_join}"
DOCUMENTS="$OUTPUT/row_documents_relarxiv.jsonl"
mkdir -p "$OUTPUT" logs
exec > >(tee -a "logs/relarxiv_index_${OAR_JOB_ID:-local}.log") 2>&1
...
FINGERPRINT=$( { sha256sum rel-arxiv_csv/*.csv rel-arxiv_csv/schema_profile_relarxiv.json build_row_index.py; printf '%s\n' "$STRATEGY"; } | sha256sum | cut -d ' ' -f 1)
...
if [[ ! -f "$DOCUMENTS" ]]; then
    "$PYTHON" build_row_index.py \
        --csv_dir rel-arxiv_csv \
        --schema_profile rel-arxiv_csv/schema_profile_relarxiv.json \
...
profile = json.loads(Path('rel-arxiv_csv/schema_profile_relarxiv.json').read_text())
