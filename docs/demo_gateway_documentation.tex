...
\begin{itemize}
  ...
  \item preserve row identifiers from source data through retrieval and generation;
  ...
  \item provide an interface for comparing the different pipelines.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Provenance}: the witness sets that explain which row identifiers
       support each tuple;
  \item \textbf{Explanation}: the human-readable explanation for the provenance.
\end{itemize}
...
\begin{enumerate}
  \item \textbf{Select a dataset.} In the \textit{Dataset} menu, choose the dataset.
  ...
  \item \textbf{Generate the plan.} Select \textit{Generate plan}. In the
        ...
        through the join and projection (and ordering when requested), to the final tuples with
        provenance.
  ...
  \item \textbf{Read the answer.} When the status becomes \textit{Complete}, use
        ...
        pipeline answer, its automatic confidence level and a view of the exact rendered prompt sent to the model.
  ...
  \item \textbf{Trace provenance.} Open the \textit{Provenance} tab. This is the
      explanation of why the result exists obtained running the query on ProvSQL.
...
In the browser execution flow, the gateway writes one CSV per contributing table
...
rows, with RAG leaf outputs checked against retrieved source rows. The
Internal-Knowledge baseline instead undergoes structural validation and does not
provide verified source evidence. Stable row identifiers then connect final answers, provenance
formulas, exported CSVs, and the external explanation
service.
...
result source, the pipeline used and any related provenance or evidence references. As
partial results are combined, these annotations are propagated and attached to the complete answer.
...
qualitative confidence level of low, medium, or high at block or subtree level, assigned by an llm.
...
individual evidence fragments. Documents built by \code{FaissIndexManager} store
\code{chunk\_id} explicitly; offline row indexes without this field fall back
to the row identifier. Final RAG confidence annotations propagate the relevant identifiers
...
into the complete pipeline result.
...
being shown only at the leaf where they were generated.
For a RAG-derived result, the final relevance score is the minimum FAISS
relevance score among the retrieved rows that contribute to that result. For an
...
rows were read and processed deterministically.
...
current UI pipeline combination.
...
associated metadata contains the table name, row identifier, and chunk identifier:
...
metadata:
{"table": "nation", "rid": "nation_2", "chunk_id": "nation_2"}
\end{lstlisting}
...
We also tried the opt-in \code{semantic-join} textualization strategy in
\code{build\_row\_index.py} as an alternative to the rich format. It emits only the
record's first 40 non-provenance fields as \code{column is value} assertions separated
by semicolons:
...
for the offline BGE-M3 workflow; other backends replace the embedding step.
The metadata follows the vector into the index and supports row resolution and
context expansion after retrieval. FAISS similarity search compares the embedded query with the embedded
...
strategy at query time. The runtime wrapper loads BGE-M3 through
\code{FlagEmbedding.BGEM3FlagModel}. The offline builders instead use
\code{HuggingFaceEmbeddings} with \code{normalize\_embeddings=True} for the
same model. These distinct paths must produce compatible document and query
embeddings.
...
query embeddings before FAISS similarity search. In the runtime BGE-M3 wrapper, documents are encoded in batches
with an explicit maximum input length of 8192 tokens; queries are encoded
individually with the same limit. The offline builders rely on the loaded
model configuration for the input-length limit.
...
the BGE v1.5 path, and other names use Sentence Transformers. When set to \code{auto}, the shared
\code{EMB\_DEVICE} setting selects CUDA when available or CPU otherwise; an
explicit device can also be configured. BGE
models use half precision on CUDA and full precision on CPU.

An index must be queried with a compatible model, normalization, and encoding
configuration. Changing the model or row serialization requires rebuilding the
corresponding FAISS index. Changing backend wrappers also requires checking
compatibility, even when the model name and vector dimensions match.

\subsubsection{Alternative Strategies Tried}

During development, we tried different ways of representing rows before
embedding: the generic compact \code{column: value} format, the schema-aware
\code{rich} format, and the \code{semantic-join} format. The latter two have
separate REL-F1 index directories, allowing comparison under the same retrieval
pipeline. Rich text includes table identity, keys, and selected linked-row
values; semantic-join tests a shorter representation containing only field
assertions. Both retain structured metadata for row resolution and provenance.

This serialization comparison is separate from the embedding-backend choice.
The implementation supports Sentence Transformers, BGE v1.5, and BGE-M3;
backend support alone does not establish that every model and serialization
combination was evaluated. The configurations described here use MPNet by
default for TPC-H and BGE-M3 for the prebuilt REL-F1 index. No quantitative
comparison is reported here, so rich text with BGE-M3 is the demo default,
not a demonstrated best-performing strategy.

\subsection{Retrieval Modes}

The direct model baseline path performs one similarity search with \code{RETRIEVER\_K}
...
A RAG leaf model must return a JSON array containing objects with exactly two
keys. This source-row contract does not apply to the structurally validated
Internal-Knowledge baseline:
