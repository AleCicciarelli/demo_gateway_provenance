...
at the shared FAISS retrieval stage. This stage is used by RAG, RAG with
predicate pushdown, RAG Explanation, and iterative join-aware RAG. Internal Knowledge is
...
JSON result and semantic provenance identifiers are validated before the
pipeline continues. In the second stage, the gateway makes a separate
...
annotations without converting unlike confidence signals into a single
...
about the correctness of the source data and is not a probability.
...
The canonical \code{llm-internal} mode performs no source-row retrieval. It asks the
...
validated. The older \code{internal-knowledge} spelling remains a compatibility
alias.
...
\subsection{RAG Explanation}

\code{rag-explanation} extends RAG extraction by writing the
...
provenance formula, and supporting rows. The legacy
\code{planner-only-explanation} name remains accepted.
